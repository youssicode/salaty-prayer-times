...
//==================
import {
  renderUpcomingPrayer,
  renderUpcomingPrayerCountDown,
  renderCallToPrayerOverlay,
} from "./dataRendering.js"
import { queryAll } from "./domElements.js"
...
export const renderUpcomingPrayerCard = (passedArray, time_zone) => {
  ...
  let dateTimeZone = new Date()
    .toLocaleString("en-US", { timeZone: time_zone, hour12: false })
    .replace(/ 24:/, " 00:") // 24(hour format) causes an 'invalid date' error, and the space befor ' 24' to avoid replacing minutes. //new Date().toString(...) Did not get the local time;
  ...
  if (actualTimeStamp > ishaaTimeStamp) {
    ...
    const dateTemplateMonth =
      date.getMonth() + 1 < 10
        ? "0" + (date.getMonth() + 1)
        : date.getMonth() + 1 // get ACTUAL month in 2-digit number
    const dateTemplateDay =
      date.getDate() + 1 < 10 ? "0" + (date.getDate() + 1) : date.getDate() + 1 // get Tomorro's day in 2-digit number
    ...
    for (let i = 0; i < timesArray.length; i++) {
      upComingPrayerTimeStamp = new Date(
        `${date.toDateString()} ${timesArray[i].prayerTime}:00`
      ).getTime()

      if (upComingPrayerTimeStamp > actualTimeStamp && i != 1) {
        // '&& i != 1' To avoid displaying 'Sunrise' as upcoming prayer
...
const startCallToPrayer = () => {
  ...
  if (
    adhanBells[prayerTimeCard__index].classList.contains(
      "prayerTimeCard__adhan--disabled"
    )
  )
    return
  renderCallToPrayerOverlay()
