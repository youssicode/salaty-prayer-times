...
export const renderTimesTableModalOverlay = () => {
    ...
    dom().monthPicker.selectedIndex = currentMonthIndex;
...
const fetchTableData = async (month) => {
    ...
    const {data} = await axios("/times-table", {params: {month, latitude, longitude}})
    return data
}
...
function mapTableData(dataTable) {
    ...
    if (day == toDay) { // note that 'day' is a string and 'toDay' is a number
