...
export async function getPrayerTimes(coords) {
  try {
    const fetchedPrayerTimes = await fetchPrayerTimesByLocationCoordinates(
      coords.latitude,
      coords.longitude
    )
    ...
    renderUpcomingPrayerCard(
      fetchedPrayerTimes,
      getDataFromLocalStorage("salaty_localTimeZone")
    )
  } catch (err) {
    errorHandler(err)
  }
...
async function fetchPrayerTimesByLocationCoordinates(x, y) {
  const { status, data } = await axios.get("/prayer-timings", {
    params: { x, y },
  })
...
export async function prayerTimesByCity(city, country) {
  const { status, data } = await axios.get("/prayer-timings-by-city", {
    params: { city, country },
  })
  if (status >= 200 && status < 300) {
    return data
...
export async function refreshPrayerTimingForChosenCity(chosenCity) {
  try {
    ...
    const { mainPrayerTimings, newCityCoords, city_time_zone, newHijriDate } =
      await prayerTimesByCity(city, country)
